...
| `surge_signals` | Volume Surge strategy signals (keyed by ticker + surge_date) with workflow `status`, drop_date, buy_level, window_days_left, trigger_date — see `surge_strategy.py` |
...
- `hk/` — standalone Hong Kong volume-surge study, deliberately isolated from the dashboard (own `hk/hk_stocks.db`, imports nothing from `api_server`/`db_setup`/`fetch_data`; only reuses the pure functions in `scripts/study_volume_surge_2026_04.py` and `scripts/capital_simulation.py`). `python hk/hk_fetch.py [--all]` downloads the Hang Seng Index (default) or every HKEX-listed equity (`--all`, from HKEX's List of Securities); `python hk/hk_study.py --universe all --min-turnover 5000000 --capital 10000` runs the study (results in `hk/runs/<tag>/`, git-ignored). A liquidity filter is essential for the full market (unfiltered, the study is dominated by untradeable penny stocks). Known bias: the ticker list is today's listings (survivorship).
